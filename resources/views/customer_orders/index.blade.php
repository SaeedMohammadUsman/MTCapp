                ...
                <tbody>
                    @forelse ($customerOrders as $customerOrder)
                        <tr>
                            ...
                            <td>{{ $customerOrder->customer->customer_name_en }} ({{ $customerOrder->customer->customer_name_fa }})</td>
                            ...
                            <td>
                                <div class="btn-group">
                                    @if ($customerOrder->trashed())
                                        ...
                                        <form action="{{ route('customer_orders.restore', $customerOrder->id) }}" method="POST" style="display:inline;">
                                            @csrf
                                            <button type="submit" class="btn btn-success btn-sm">Restore</button>
                                        </form>
                                    @else
                                        {{-- View, Edit, Delete buttons for active orders --}}
                                        <a href="{{ route('customer_orders.show', $customerOrder->id) }}" class="btn btn-info btn-sm">View</a>
                                        <a href="{{ route('customer_orders.edit', $customerOrder->id) }}" class="btn btn-warning btn-sm">Edit</a>

                                        <form action="{{ route('customer_orders.destroy', $customerOrder->id) }}" method="POST" id="delete-form-{{ $customerOrder->id }}" style="display:inline;">
                                            @csrf
                                            @method('DELETE')
                                            <button type="button" onclick="confirmDelete(event, 'delete-form-{{ $customerOrder->id }}')" class="btn btn-danger btn-sm">Delete</button>
                                        </form>
                                    @endif
                                </div>
                            </td>
...
<div class="modal fade" id="createCustomerOrderModal" tabindex="-1" role="dialog" aria-labelledby="createCustomerOrderModalLabel"
    aria-hidden="true">
